#!/home/grbrum/Work/3d_print/.venv/bin/python
...
def create_item():
    ...
    try:
        ...
        if response.status_code == 200:
            messagebox.showinfo("Success", f"Product created: {response.json().get('sku')}")
            clear_form()
        else:
...
tk.Checkbutton(root, text="Production", variable=var_production).grid(row=2, column=1, sticky="w")
